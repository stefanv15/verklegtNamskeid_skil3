...
void RelationWindow::displayScientist(vector<Person> persons)
{
    ...
    ui->table_relation_schientists->setColumnCount(5);
    ...
    ui->table_relation_schientists->setRowCount(persons.size());
    ...
    for(unsigned int i = 0; i < persons.size(); i++)
    {
       // Person currentScientist = persons[i];
        ui->table_relation_schientists->setItem(i, 0, new QTableWidgetItem(QString::fromStdString(persons[i].getName())));
        ui->table_relation_schientists->setItem(i, 1, new QTableWidgetItem(QString::fromStdString(persons[i].getGender() == "m"?"Male":"Female")));
        ui->table_relation_schientists->setItem(i, 2, new QTableWidgetItem(QString::number(persons[i].getDayOfBirth())));
        if (persons[i].getDayOfDeath() > 0)
            ui->table_relation_schientists->setItem(i, 3, new QTableWidgetItem(QString::number(persons[i].getDayOfDeath())));
        else
            ui->table_relation_schientists->setItem(i, 3, new QTableWidgetItem(QString::fromStdString("Still alive")));
        ui->table_relation_schientists->setItem(i, 4, new QTableWidgetItem(QString::fromStdString(m_domain.getComputerList(persons[i].getId()))));
        ui->table_relation_schientists->setItem(i, 5, new QTableWidgetItem(QString::number(persons[i].getId())));
